...
class GraphCollator:
    ...
    def __init__(
        ...
        *,
        ...
        self.cg_cache = CGCache(
            cap=int(cg_cache * graph.num_node),
    ...
    def __call__(self, batch: List[Tuple[int, int, int, float, int, int]]):
        src_ids, dst_ids, neg_dst_ids, tss, eids, labels = (
        ...
        with record_function("cg sampling"):
            ...
            src_ids, dst_ids, neg_dst_ids, eids, labels = (
                ...
                for x in (src_ids, dst_ids, neg_dst_ids, eids, labels)
            )
            tss = torch.from_numpy(tss).float()

        return src_ids, dst_ids, neg_dst_ids, tss, eids, labels, computation_graph
    ...
    def collate_memory_nodes(self, nids: np.ndarray, tss: np.ndarray) -> Tuple:
        layers: List = [[] for _ in range(self.n_layers + 1)]
        ...
        if nids[0] > 1000:
            ...
            for i in range(len(layers)):
                msg += f"{i} -> {(layers[i][0]).shape}, {type(layers[i][0])}\n"
                if i != 0:
    ...
    def _collate_memory_nodes_recursive(
        self, nids: np.ndarray, ts: np.ndarray, depth: int, layers: List
    ):
        if depth == 0:
            return
        neigh_nids, neigh_eids, neigh_ts, _, neigh_re, neigh_degree = (
            self.graph.sample_temporal_neighbor(nids, ts, self.n_neighbors)
        )  # inputs and outputs are all np.ndarray
        layers[depth] = (neigh_nids, neigh_eids, neigh_ts, neigh_re, neigh_degree)
        self._collate_memory_nodes_recursive(
    ...
    def collate_history(self, nids: np.ndarray, ts: np.ndarray) -> SeqRestartData:
        # de-duplicate
        unique_nids, index = select_latest_nids(
            torch.from_numpy(nids), torch.from_numpy(ts)
        )
...
class InteractionData(Dataset):
    ...
    def get_subset(self, start, end):
        ...
        ds = InteractionData(
            src, dst, ts, eids, labels, self.seed, self.eval, self.neg_dst
        )
        return ds
...
class RandEdgeSampler:
    def __init__(
        self, src_list: np.ndarray, dst_list: np.ndarray, seed: Optional[int] = None
    ):
        self.seed = seed
...
def load_jodie_data(
    name: str, train_seed: int, *, root=".", data_seed=2020, val_p=0.7, test_p=0.85
) -> Tuple[
    Optional[np.ndarray],
    Optional[np.ndarray],
    float,
    ...
    shift_max = compute_delta_max(src, dst, ts)
    ...
    part_inductive_node_set = set(
        random.sample(list(inference_node_set), int(0.1 * n_total_nodes))
    )
    ...
    inductive_edge_mask = np.isin(src, list(inductive_node_set)) | np.isin(
        dst, list(inductive_node_set)
    )
    ...
    val_data = InteractionData(
        *[x[val_data_mask] for x in [src, dst, ts, eids, labels]], seed=0, eval=True
    )
    test_data = InteractionData(
        *[x[test_data_mask] for x in [src, dst, ts, eids, labels]], seed=2, eval=True
    )
...
def compute_delta_max(srcs: np.ndarray, dsts: np.ndarray, ts: np.ndarray) -> float:
    """
