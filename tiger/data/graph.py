...
class Graph:
    ...
    def find_unique_before(
        ...
        _, unique_idx = np.unique(
            neighbors[::-1], return_index=True
        )  # find unique nodes' indices
        unique_idx = (
            len(neighbors) - 1 - np.sort(unique_idx)[::-1]
        )  # np.unique sorts unique values
        neighbors, eids, ts, flags = [
            x[unique_idx] for x in (neighbors, eids, ts, flags)
        ]
        return neighbors, eids, ts, flags

    def sample_temporal_neighbor(
        self,
        nids: np.ndarray,
        ts: np.ndarray,
        n_neighbors: int = 10,
        ...
        batch_re = np.zeros([bs, n_neighbors], dtype=np.float32)
        ...
        for i, (nid, t) in enumerate(zip(nids, ts)):
            ...
            neighbor_unique_keys, neighbor_inverse_indices, neighbor_counts = np.unique(
                local_neighbors, return_inverse=True, return_counts=True
            )

            # find re-occurrence number for each neighbor
            # ndarray, shape (num_neighbors, )
            local_re = neighbor_counts[neighbor_inverse_indices].astype(np.float32)
            node_degree = len(local_neighbors)
            ...
            if strategy == "uniform" or strategy == "time":
                ...
                unique_idx = (
                    len(local_neighbors) - 1 - np.sort(unique_idx)[::-1]
                )  # np.unique sorts unique values
        ...
        return batch_neighbors, batch_eids, batch_ts, batch_dirs, batch_re, batch_degree

    def get_history(
        self, nids: np.ndarray, ts: np.ndarray, hist_len: int
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        hist_nids, hist_eids, hist_ts, hist_dirs, *_ = self.sample_temporal_neighbor(
    ...
    def sample_walks(
        self, nids: np.ndarray, ts: np.ndarray, n: int, length: int, alpha: float
...
def data2adjlist(
    data: "InteractionData", max_node_id: Optional[int] = None
