...
class ComputationGraph:
    ...
    def __init__(
        self,
        tige_data: Tuple[List[Tuple], Tensor],
    ...
    def to(self, device: torch.device):
        for depth in range(len(self.layers)):
            neigh_nids, neigh_eids, neigh_ts, neigh_re, neigh_degree = self.layers[
                depth
            ]
            ...
            if depth > 0:
                ...
                neigh_re = neigh_re.to(device)
                neigh_degree = neigh_degree.to(device)
            self.layers[depth] = (
                neigh_nids,
                neigh_eids,
                neigh_ts,
    ...
    def pin_memory(self):
        for depth in range(len(self.layers)):
            ...
            if depth > 0:
                ...
                neigh_re = neigh_re.pin_memory()
                neigh_degree = neigh_degree.pin_memory()
            self.layers[depth] = (
                neigh_nids,
                neigh_eids,
                neigh_ts,
