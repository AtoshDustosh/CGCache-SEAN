...
from .memory import Memory, MessageStore, MessageStoreNoGrad, MessageStoreNoGradLastOnly
...
class TIGE(nn.Module):
    def __init__(
        ...
        *,
        ...
        msg_src: str,
        upd_src: str,
        msg_tsfm_type: str = "id",
        mem_update_type: str = "gru",
        tgn_mode: bool = True,
        msg_last_only: bool = True,
        hit_type: str = "none",
        importance_dim: int = 100,
        shift_max: float = 0.0,
        threshold: float = 0.0,
        adl_control: float = 1.0,
    ):
        """
        ...
        if self.msg_last_only:
            # Use this!!! Fastest!!!
            self.msg_store = MessageStoreNoGradLastOnly(
                self.n_nodes, dim=self.raw_msg_dim
            )
        elif self.tgn_mode:
        ...
        self.msg_memory = (
            self.left_memory if self.msg_src == "left" else self.right_memory
        )
        self.upd_memory = (
            self.left_memory if self.upd_src == "left" else self.right_memory
        )
        ...
        if msg_tsfm_type == "id":
            self.msg_transform_fn = IdentityMessageFunction(
        ...
        self.score_fn = MergeLayer(
            merge_dim, merge_dim, self.nfeat_dim, 1, dropout=dropout
        )
    ...
    def contrast_learning(
        ...
        """
        bs = len(src_ids)
        pos_node_ids = torch.cat(
            [src_ids, dst_ids]
        )  # we only update states of these nodes
        ...
        if len(outdated_nids):
            h_prev_right_all = self.apply_messages(
                outdated_nids, msgs, prev_ts
            )  # h(t'+)
            ...
            involved_node_reprs[involved_local_index] = h_prev_right_all[
                outdated_local_index
            ]

        # STEP 3: compute temporal embeddings h(t-) using h(t'+), and attentions
        ...
        attentions = self.temporal_embedding_fn.attentions
        ...
        # h(t'-) and h(t'+)
        h_prev_left, _ = self.left_memory.get(
            pos_node_ids
        )  # old left memory for restarter
        ...
        adl_loss = self.ADLoss(attentions, threshold=self.threshold)

        total_loss = contrast_loss + adl_loss * self.adl_control

        return total_loss, h_left, pos_scores, neg_scores, h_prev_left, h_prev_right

    def ADLoss(self, attentions: list, threshold: float):
        """
        compute the ADLoss of an attention maps
        :param attentions: list, size (num_layers, [Tensor])
        ...
        for i in range(len(attentions)):
            # 应该是可以堆叠的，因为同一个batch中的batch_size都相同
            # Tensor, shape (num_update, batch_size, num_neighbors)
            attention_score = torch.stack(attentions[i], dim=0)

            # Tensor, shape (num_update, batch_size, 1)
            attention_avg = torch.mean(attention_score, dim=2, keepdim=True)

            # Indicator Function
            # Tensor, shape (num_update, batch_size, num_neighbors and 1)
            attention_score = (attention_score > threshold) * attention_score
            attention_avg = (attention_avg > threshold) * attention_avg

            # Tensor, shape (num_update, batch_size)
            sim = F.cosine_similarity(attention_avg, attention_score, dim=-1)
            sim = sim.mean()

            sim_sum += sim
            count += 1

        sim_sum = sim_sum / count
        return sim_sum

    def compute_messages(
        self, node_ids: Union[Tensor, np.ndarray, None] = None
        ...
        raw_msgs, ts = self.msg_aggregate_fn(
            outdated_nodes, last_update_ts, self.msg_store.node_messages
        )
        # Sanity check  TODO: to remove
        if self.msg_src == "left" and not (ts == last_update_ts).all().item():
            raise ValueError(
                "Messages' ts should be equal to last update ts "
    ...
    def compute_temporal_embedding_with_involved_nodes_only(
        ...
        -----
        involved_node_reprs: h(t'+) of involved nodes, Tensor, shape ()
        ...
        """
        self.temporal_embedding_fn.init_para_for_one_batch()
    ...
    def temporal_embedding(
        self, memory: Memory, node_ids: Tensor, ts: Tensor
    ) -> Tensor:
        """
        ...
        """
        warnings.warn(
            'Please use "compute_embedding_with_computation_graph" instead!',
            DeprecationWarning,
        )
        # temporal embedding fn requires np.ndarray as inputs
        h = self.temporal_embedding_fn.compute_embedding(
            all_node_reprs=memory.vals,
    ...
    def _get_computation_graph_nodes(
        ...
        """
        warnings.warn("This method is no longer usefule!", DeprecationWarning)
        depth = self.n_layers if depth is None else depth
        if depth == 0:
            return np.unique(nids)
        ngh_nids, _, neigh_ts, *_ = self.graph.sample_temporal_neighbor(
            nids, ts, self.n_neighbors
        )
        r_nids = self._get_computation_graph_nodes(
            ngh_nids.flatten(), neigh_ts.flatten(), depth - 1
        )
...
class TIGER(TIGE):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            ...
            msg_src=msg_src,
            upd_src=upd_src,
            msg_tsfm_type=msg_tsfm_type,
            mem_update_type=mem_update_type,
            tgn_mode=tgn_mode,
            msg_last_only=msg_last_only,
            hit_type=hit_type,
            importance_dim=importance_dim,
            shift_max=shift_max,
            threshold=threshold,
    ...
    def contrast_and_mutual_learning(
        ...
        """
        contrast_loss, *_, h_prev_left, h_prev_right, attentions = (
            self.contrast_learning(
        ...
        if len(valid_rows):
            mutual_loss = self.mutual_loss_fn(
                preds[valid_rows], targets[valid_rows].detach()
            )
        else:
            mutual_loss = torch.tensor(0, device=contrast_loss.device)
    ...
    @torch.no_grad()
    def restart(self, nids: Tensor, ts: Tensor, mix: float = 0.0):
        """
        ...
        if len(nids):
            ...
            if mix > 0:
                h_prev_left = (
                    mix * h_prev_left + (1 - mix) * self.left_memory.vals[nids]
                )
                h_prev_right = (
                    mix * h_prev_right + (1 - mix) * self.right_memory.vals[nids]
                )
