...
class MessageStoreNoGradLastOnly(nn.Module):
    ...
    def __init__(self, n, dim):
        ...
        self.register_buffer(
            "node_msg_vals", torch.zeros((n, dim)).float(), persistent=False
        )
        self.register_buffer("node_msg_ts", torch.zeros(n).float(), persistent=False)
    ...
    def store_events(
        ...
        src_full_vals = torch.cat(
            [src_vals, dst_vals, edge_vals, time_encoder(ts - src_prev_ts)], 1
        )  # [n, 4d]
        dst_full_vals = torch.cat(
            [dst_vals, src_vals, edge_vals, time_encoder(ts - dst_prev_ts)], 1
        )  # [n, 4d]

        # print("src_ids", src_ids.shape, src_ids.device)
        # print("dst_ids", dst_ids.shape, dst_ids.device)
        # print("ts", ts.shape, ts.device)
        # print("src_full_vals", src_full_vals.shape, src_full_vals.device)
        # print("dst_full_vals", dst_full_vals.shape, dst_full_vals.device)
        # print("ts.repeat(2).shape", ts.repeat(2).shape)
        # print("concat ids shape", torch.cat([src_ids, dst_ids]).shape)
        full_nids, index = select_latest_nids(
            torch.cat([src_ids, dst_ids]), ts.repeat(2)
        )
        # print("full_nids.shape", full_nids.shape, full_nids.device)
        # print("index.shape", index.shape, index.device)
        # print("index min/max", index.min().item(), index.max().item())
        # print(
        #     "concat values len",
        #     torch.cat([src_full_vals, dst_full_vals], dim=0).shape[0],
        # )
    ...
    def get_outdated_node_ids(
        self, node_ids: Union[Tensor, np.ndarray, None]
    ) -> Tensor:
        """
        ...
        if node_ids is None:
            ...
            if isinstance(node_ids, np.ndarray):
                ...
                outdated_node_ids = self.nodes_with_messages & set(
                    node_ids.cpu().numpy()
                )
    ...
    def clear(self, nids: Optional[Tensor] = None):
        ...
        if nids is None:
            nids = torch.tensor(
                list(self.nodes_with_messages), device=self.node_msg_vals.device
            ).long()
...
class MessageStore:
    ...
    def store_events(
        ...
        src_ids, dst_ids, ts, eids = (
            x.cpu().numpy() for x in (src_ids, dst_ids, ts, eids)
        )
        # sanity check
        # for these nodes, their messages should be used and thus cleared
        # TODO: To remove this check
        for n in np.concatenate([src_ids, dst_ids]):
            if len(self.node_messages[n]):
                raise ValueError(
                    f"Node #{n} has {len(self.node_messages[n])} unused messages."
                )
        ...
        for src, dst, val, eid, t in zip(src_ids, dst_ids, vals, eids, ts):
            ...
            self.node_messages[dst].append(
                (val.roll(shifts=1, dims=0).flatten(), eid, t, False)
            )
...
class MessageStoreNoGrad(MessageStore):
    ...
    def store_events(
        ...
        ts = ts.cpu().numpy()
        for src, dst, sval, dval, t in zip(
            src_ids, dst_ids, src_full_vals, dst_full_vals, ts
        ):
