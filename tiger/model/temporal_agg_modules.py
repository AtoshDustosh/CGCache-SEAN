...
class GraphEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.importance_encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Linear(
                in_features=self.importance_dim, out_features=self.importance_dim
            ),
        )
    ...
    def init_para_for_one_depth(self, batch_size, time_feat_dim, node_feat_dim):
        self.hidden = torch.zeros(
            (1, batch_size, 4 * (node_feat_dim + time_feat_dim))
        ).to(self.device)
    ...
    def compute_embedding_with_computation_graph(
        ...
        """
        depth = (
            self.n_layers if depth is None else depth
        )  # current depth, from n_layers to 0
        ...
        center_node_reprs = involved_node_reprs[
            local_center_nids
        ...
        #
        neigh_nids, neigh_eids, neigh_ts, neigh_re, neigh_degree = (
            computation_graph.layers[depth]
        )
        ...
        neigh_importance_features = self.importance_encoder(
            importance_score.unsqueeze(2).float().to(self.device)
        )
    ...
    def compute_embedding(
        self,
        all_node_reprs: Tensor,
        np_center_nids: np.ndarray,
        np_ts: np.ndarray,
        depth: Optional[int] = None,
    ) -> Tensor:
        """
        ...
        center_node_reprs = all_node_reprs[
            center_nids
        ] + self.raw_feat_getter.get_node_embeddings(center_nids)
        ...
        np_neigh_nids, np_neigh_eids, np_neigh_ts, *_ = (
            self.graph.sample_temporal_neighbor(np_center_nids, np_ts, self.n_neighbors)
        )  # inputs and outputs are all np.ndarray
        # filter and compress?
        if False:
            # remove all-padding columns. at least 1 column is kept to avoid bugs.
            np_neigh_nids, np_neigh_eids, np_neigh_ts = filter_neighbors(
                np_neigh_nids, np_neigh_eids, np_neigh_ts
            )

            n_center, n_neighbors = (
                np_neigh_nids.shape
            )  # maybe n_neighbors < self.n_neighbors for reducing computation
            ...
            neigh_reprs = self.compute_embedding(
                all_node_reprs=all_node_reprs,
        ...
        edge_feats = self.raw_feat_getter.get_edge_embeddings(neigh_eids)  # 3D
        delta_ts = (
            torch.from_numpy(np_ts[:, None] - np_neigh_ts).float().to(self.device)
        )
        delta_ts_reprs = self.time_encoder(delta_ts)  # 3D
        t0_reprs = self.time_encoder(torch.zeros_like(delta_ts[:, 0]))  # 2D
        ...
        center_node_reprs = self.aggregate(
            depth=depth,
            center_x=center_node_reprs,
            center_tx=t0_reprs,
            neigh_x=neigh_reprs,
...
class GraphAttnEmbedding(GraphEmbedding):
    ...
    def aggregate(
        ...
        hidden: Tensor,
        update_prob: Tensor,
        cum_update_prob: Tensor,
    ):
        fn = self.fns[self.n_layers - depth]
        h, attention, hidden, cell, update_prob, cum_update_prob = fn(
            node_features=center_x,
            node_time_features=center_tx,
            neighbor_node_features=neigh_x,
            neighbor_node_edge_features=edge_x,
            neighbor_node_time_features=edge_tx,
            neighbor_masks=mask,
            neighbor_node_importance_features=neigh_imp,
...
class TemporalAttention(nn.Module):
    def __init__(
        ...
        self.score_linear = nn.Linear(
            self.head_dim * self.num_neighbors, self.num_neighbors
        )
        ...
        self.short_gate = nn.Sequential(
            nn.Linear(4 * self.query_dim, 4 * self.query_dim, bias=True), nn.Tanh()
        )
    ...
    def forward(
        ...
        neighbor_node_importance_features: torch.Tensor,
        time_interval: torch.Tensor,
        cell: torch.Tensor,
        hidden: torch.Tensor,
        update_prob: torch.Tensor,
        cum_update_prob: torch.Tensor,
        ...
        importance_score = torch.tanh(
            torch.cat(
                [
                    neighbor_node_importance_features,
        ...
        query = self.query_projection(query).reshape(
            query.shape[0], query.shape[1], self.n_head, self.head_dim
        )
        ...
        key = self.key_projection(key).reshape(
            key.shape[0], key.shape[1], self.n_head, self.head_dim
        )
        # Tensor, shape (batch_size, num_neighbors, num_heads, self.head_dim)
        value = self.value_projection(value).reshape(
            value.shape[0], value.shape[1], self.n_head, self.head_dim
        )
        # Tensor, shape (batch_size, num_neighbors, num_heads, self.head_dim)
        importance_score = self.importance_projection(importance_score).reshape(
            importance_score.shape[0],
        ...
        importance_score = importance_score.reshape(
            ...
            importance_score.shape[1],
            1,
            importance_score.shape[2] * importance_score.shape[3],
        )
        ...
        output, hidden_output, cell_output, update_prob, cum_update_prob = (
            self.skip_time_decay_depth_selector(
                input=output,
                cell=cell,
                hidden=hidden,
    ...
    def skip_time_decay_depth_selector(
        self,
        input: Tensor,
        cell: Tensor,
        hidden: Tensor,
        cum_update_prob_prev: Tensor,
        update_prob_prev: Tensor,
        time_interval: Tensor,
    ):
        """
        skip depth selector
        ...
        output, (hidden_output, cell_output) = self.lstm(
            depth_input, (hidden, cell_new)
        )
        ...
        cum_update_prob = cum_update_prob_prev + torch.min(
            update_prob_prev, 1.0 - cum_update_prob_prev
        )  # 这个batch原来的u_t
        # round
        bn = BinaryLayer()
        update_gate = bn.apply(cum_update_prob)
        # apply update gate
        new_c = update_gate * cell_output + (1.0 - update_gate) * cell_new
        new_h = update_gate * hidden_output + (1.0 - update_gate) * hidden
        # 如果更新了，则为delta；没更新，和上一个batch原来的u_t一样
        new_update_prob = (
            update_gate * new_update_prob_tilde + (1.0 - update_gate) * update_prob_prev
        )
...
def filter_neighbors(
    ngh_nids: np.ndarray, ngh_eids: np.ndarray, ngh_ts: np.ndarray
...
def compress_node_ts_pairs(
    nids: np.ndarray, ts: np.ndarray
) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
