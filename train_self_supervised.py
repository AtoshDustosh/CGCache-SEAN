    ...
    *,
    ...
    try:
        ...
        model = init_model(
            ...
            full_data,
            device,
            feature_as_buffer=feature_as_buffer,
            dim=dim,
            n_layers=n_layers,
            n_heads=n_heads,
            n_neighbors=n_neighbors,
            ...
            imporatance_dim=importance_dim,
            shift_max=shift_max,
            threshold=threshold,
        ...
        for epoch in range(epoch_start, n_epochs):
            ...
            with prof_ctx as prof:
                ...
                for i_batch, (
                    ...
                    if restarting:  # in lazy mode
                        ...
                        model.restart(
                            r_nids,
                            torch.full((len(r_nids),), ts.min().item()).to(device),
                        )
            ...
            memory_state_train_end = (
                model.save_memory_state()
            )  # save states at t_train_end
            ...
            memory_state_valid_end = (
                model.save_memory_state()
            )  # save states at t_valid_end
            model.load_memory_state(
                memory_state_train_end
            )  # load states at t_train_end
        ...
        logger.info(
            f"[ Val] Best     ap: {    best_val_ap:.4f} Best     auc: {    best_val_auc:.4f}"
        )
        logger.info(
            f"[ Val] Best ind ap: {best_ind_val_ap:.4f} Best ind auc: {best_ind_val_auc:.4f}"
        )
        ...
        test_ap, test_auc = eval_edge_prediction(
            model, test_dl, device, restart_mode, uptodate_nodes=uptodate_nodes.copy()
        )  # memory modified
        ...
        results.update(
            ...
            val_ap=best_val_ap,
            ind_val_ap=best_ind_val_ap,
            val_auc=best_val_auc,
            ind_val_auc=best_ind_val_auc,
            test_ap=test_ap,
            test_auc=test_auc,
            ind_test_ap=ind_test_ap,
            ind_test_auc=ind_test_auc,
...
def get_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Dimension of importance features",
    )
    parser.add_argument(
        "--th", type=float, default=0.1, help="Threshold of indicator function"
    )
    parser.add_argument("--adl", type=float, default=0.1, help="coef of ADL loss")
...
if __name__ == "__main__":
    ...
    run(
        ...
        importance_dim=args.importance_dim,
        threshold=args.th,
        adl_control=args.adl,
